...
public class PassthroughLayer : MonoBehaviour
{
    ...
    /// </summary>
    public PassthroughOverlay.OverlayType overlayType = PassthroughOverlay.OverlayType.Overlay;
    ...
    /// When `updateTransform` parameter is set to `true`, PassthroughOverlay will update the transform
    ...
    private Matrix4x4 GetTransformMatrixForPassthroughSurfaceObject(GameObject obj)
    {
        ...
        {
            _cameraRig = GetComponentInParent<XROrigin>();
            cameraRigInitialized = true;
        }

        Matrix4x4 trackingSpaceFromWorld = (_cameraRig != null) ?
            _cameraRig.TrackablesParent.worldToLocalMatrix :
            Matrix4x4.identity;
    ...
    {
        Debug.Assert(_passthroughOverlay != null);
        Debug.Assert(_passthroughOverlay.layerId > 0);
        meshHandle = 0;
        instanceHandle = 0;
        ...
        if (!OVRPlugin.CreateInsightTriangleMesh(_passthroughOverlay.layerId, vertices, triangles, out meshHandle))
        {
            Debug.LogWarning("Failed to create triangle mesh handle.");
            return false;
        }

        if (!OVRPlugin.AddInsightPassthroughSurfaceGeometry(_passthroughOverlay.layerId, meshHandle, T_worldInsight_model, out instanceHandle))
        {
    ...
    private void SyncToOverlay()
    {
        ...
        _passthroughOverlay.currentOverlayType = overlayType;
        _passthroughOverlay.compositionDepth = compositionDepth;
        _passthroughOverlay.hidden = hidden;
        _passthroughOverlay.overridePerLayerColorScaleAndOffset = overridePerLayerColorScaleAndOffset;
        _passthroughOverlay.colorScale = colorScale;
        _passthroughOverlay.colorOffset = colorOffset;
        _passthroughOverlay.previewInEditor = true;

        if (_passthroughOverlay.currentOverlayShape != overlayShape)
        {
            if (_passthroughOverlay.layerId > 0)
            {
            ...
            _passthroughOverlay.currentOverlayShape = overlayShape;
        }
        ...
        _passthroughOverlay.enabled = PassthroughController.Instance != null; /*&&
    ...
    private PassthroughOverlay _passthroughOverlay;
    ...
    private PassthroughOverlay.OverlayShape overlayShape
    {
        get
        {
            return projectionSurfaceType == ProjectionSurfaceType.UserDefined ?
                PassthroughOverlay.OverlayShape.SurfaceProjectedPassthrough :
                PassthroughOverlay.OverlayShape.ReconstructionPassthrough;
        }
    }
    #endregion
    ...
    void LateUpdate()
    {
        ...
        if (styleDirty)
        {
            ...
            OVRPlugin.SetInsightPassthroughStyle(_passthroughOverlay.layerId, style);
    ...
    void OnEnable()
    {
        ...
        // Create auxiliary GameObject which contains the PassthroughOverlay component for the proxy layer (and possibly other
        // auxiliary layers in the future).
        auxGameObject = new GameObject("PassthroughLayer auxiliary GameObject");
        ...
        // Add PassthroughOverlay component for the passthrough proxy layer.
        _passthroughOverlay = auxGameObject.AddComponent<PassthroughOverlay>();
        _passthroughOverlay.currentOverlayShape = overlayShape;
        SyncToOverlay();

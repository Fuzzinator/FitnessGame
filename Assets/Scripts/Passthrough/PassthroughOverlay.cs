...
using XRDevice = OVRManager.XRDevice;
...
public class PassthroughOverlay : MonoBehaviour
{
    ...
    public static PassthroughOverlay[] instances = new PassthroughOverlay[maxInstances];
    ...
    private void SetupEditorPreview()
    {
        ...
        if (previewInEditor && previewObject == null)
        {
            ...
            OverlayMeshGenerator generator = previewObject.AddComponent<OverlayMeshGenerator>();
    ...
    {
        var loadedXRDevice = XRDevice.Unknown;
#if USING_XR_SDK
...
#endif
            if (loadedXRDeviceName == "OpenVR")
            {
                loadedXRDevice = XRDevice.OpenVR;
            }
        }
        return loadedXRDevice;
    }

    void InitOVROverlay()
    {

        constructedOverlayXRDevice = XRDevice.Unknown;
        if (GetCurrentXRDevice() == XRDevice.OpenVR)
        {
        ...
        constructedOverlayXRDevice = GetCurrentXRDevice();
    ...
    void OnDisable()
    {
        ...
        var currentDevice = GetCurrentXRDevice();
        if (currentDevice != constructedOverlayXRDevice)
            return;

        if (currentDevice == XRDevice.Oculus)
        {
            DestroyLayerTextures();
            DestroyLayer();
        }
        else if (currentDevice == XRDevice.OpenVR)
        {
    ...
    private XRDevice constructedOverlayXRDevice;
    ...
    void LateUpdate()
    {
        ...
        if (currentDevice == XRDevice.OpenVR)
        {

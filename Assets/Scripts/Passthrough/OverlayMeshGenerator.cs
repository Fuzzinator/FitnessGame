...
/// <summary>
/// When attached to a GameObject with an PassthroughOverlay component, OverlayMeshGenerator will use a mesh renderer
/// to preview the appearance of the Overlay as it would appear as a TimeWarp overlay on a headset.
/// </summary>
...
public class OverlayMeshGenerator : MonoBehaviour
{
    ...
    private PassthroughOverlay _Overlay;
    ...
    private PassthroughOverlay.OverlayShape _LastShape;
    ...
    public void SetOverlay(PassthroughOverlay overlay)
    {
        _Overlay = overlay;
    }
    ...
    private void Update()
    {
        ...
        if (_Overlay)
        {
            PassthroughOverlay.OverlayShape shape = _Overlay.currentOverlayShape;
    ...
    private void UpdateMesh(PassthroughOverlay.OverlayShape shape, Vector3 position, Quaternion rotation, Vector3 scale, Rect rect)
    {
        if (_MeshFilter)
        {
            if (_Mesh == null)
    ...
    public static void GenerateMesh(List<Vector3> verts, List<Vector2> uvs, List<int> tris, PassthroughOverlay.OverlayShape shape, Vector3 position, Quaternion rotation, Vector3 scale, Rect rect)
    {
        switch (shape)
        {
            case PassthroughOverlay.OverlayShape.Equirect:
                BuildSphere(verts, uvs, tris, position, rotation, scale, rect);
                break;
            case PassthroughOverlay.OverlayShape.Cubemap:
            case PassthroughOverlay.OverlayShape.OffcenterCubemap:
                BuildCube(verts, uvs, tris, position, rotation, scale);
                break;
            case PassthroughOverlay.OverlayShape.Quad:
                BuildQuad(verts, uvs, tris, rect);
                break;
            case PassthroughOverlay.OverlayShape.Cylinder:

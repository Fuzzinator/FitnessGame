...
Shader "Triplebrick/Base_Normal_Blend"
{
    Properties
    {
        //[NoScaleOffset]_EnvTextures("env_Textures", 2DArray) = "" {}
    ...
    #define UNITY_BARE_SAMPLER(n) GLES2UnsupportedSamplerState n
            ...
            TEXTURE2D_ARRAY(env_Textures);
            SAMPLER(samplerenv_Textures);
            ...
            half4 frag(VertexOutput IN) : SV_Target
            {
                ...
                UnityTexture2DArray textureArray = UnityBuildTexture2DArrayStruct(env_Textures);
                //float4 roughness = PLATFORM_SAMPLE_TEXTURE2D_ARRAY(textureArray.tex, textureArray.samplerstate, uv4_DirtRoughness, 0);
                float4 roughness = tex2D(_DirtRoughness, uv4_DirtRoughness);
                float dirtStrength = clamp(pow(roughness.g, _BaseDirtStrength), 0.0, 1.0);
                float4 dirtColor = lerp(_BaseDirtColor, float4(1, 1, 1, 0), dirtStrength);
                float4 lerpedColor = lerp(_BaseColor, _BaseColorOverlay, roughness.r);
                float2 uv_DetailMask = IN.ase_texcoord7.zw * _DetailMask_ST.xy + _DetailMask_ST.zw;
                //float4 detailMask = PLATFORM_SAMPLE_TEXTURE2D_ARRAY(textureArray.tex, textureArray.samplerstate, uv_DetailMask, 2);
                float4 detailMask = tex2D( _DetailMask, uv_DetailMask );
                float4 lerpedDetailColor = lerp(lerpedColor, _DetailColor, ceil((1.0 - detailMask.b) + -0.95));
                float tempCeil = ceil((detailMask.b + -0.8));
                float4 reLerpedDetailColor = lerp(lerpedDetailColor, float4(float3(1, 0.95, 0.9), 0.0), tempCeil);
                float clampedDetailR = clamp(pow((detailMask.r + 0.55), 6.0), 0.0, 1.0);
                float4 lerpedMaskedDetailColor = lerp(reLerpedDetailColor, (reLerpedDetailColor * clampedDetailR), _DetailDirtStrength);
                float clampedDetailMask = clamp(((detailMask.r + -0.55) * 2.0), 0.0, 1.0);
                float4 clampResult101 = clamp((clampedDetailMask + lerpedMaskedDetailColor), float4(0, 0, 0, 0), float4(1, 1, 1, 0));
                float4 edgeWear = lerp(lerpedMaskedDetailColor, clampResult101, _DetailEdgeWear);

                float2 uv_DetailNormal = IN.ase_texcoord7.zw * _DetailNormal_ST.xy + _DetailNormal_ST.zw;
                //float3 detailNormal = UnpackNormalScale(PLATFORM_SAMPLE_TEXTURE2D_ARRAY(textureArray.tex, textureArray.samplerstate, uv_DetailNormal, 3),1.0f);
                float3 detailNormal = UnpackNormalScale(tex2D( _DetailNormal, uv_DetailNormal ), 1.0f );
                float2 uv4_BaseNormal = IN.ase_texcoord7.xy * _BaseNormal_ST.xy + _BaseNormal_ST.zw;
                //float3 baseNormal = lerp(float3(0, 0, 1),UnpackNormal(PLATFORM_SAMPLE_TEXTURE2D_ARRAY(textureArray.tex, textureArray.samplerstate, uv4_BaseNormal, 1)),_BaseNormalStrength);
                float3 baseNormal = lerp(float3(0, 0, 1),UnpackNormal(tex2D( _BaseNormal, uv4_BaseNormal )) , _BaseNormalStrength);

                float lerpedDetailMask = lerp(0.0, clampedDetailMask, _DetailEdgeWear);
                float clampedTempDetailMask = clamp((lerpedDetailMask + tempCeil), 0.0, 1.0);
                float baseMetallic = clamp((clampedTempDetailMask + _BaseMetallic), 0.0, 1.0);

                float4 temp_cast_2 = (_DetailEdgeSmoothness).xxxx;
                float4 lerpResult121 = lerp(((max(clampedDetailMask, roughness.a) * dirtColor) * _BaseSmoothness),
                                            temp_cast_2, clampedTempDetailMask);

                float3 Albedo = (dirtColor * edgeWear).rgb;
                float3 Normal = BlendNormal(detailNormal, baseNormal);
                float3 Emission = 0;
                float3 Specular = 0.5;

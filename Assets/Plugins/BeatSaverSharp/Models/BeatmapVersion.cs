...
namespace BeatSaverSharp.Models
{
    ...
    public class BeatmapVersion : BeatSaverObject, IEquatable<BeatmapVersion>
    {
        ...
        public UniTask<VoteResponse> Vote(Vote.Type voteType, Vote.Platform platform, string platformID, string proof, CancellationToken token = default)
        {
            return Client.Vote(Hash, voteType, platform, platformID, proof, token);
        }

        public UniTask<byte[]?> DownloadZIP(CancellationToken token = default, IProgress<double>? progress = null)
        {
            return Client.DownloadZIP(this, token, progress);
        }

        public UniTask<byte[]?> DownloadCoverImage(CancellationToken token = default, IProgress<double>? progress = null)
        {
            return Client.DownloadCoverImage(this, token, progress);
        }

        public UniTask<byte[]?> DownloadPreview(CancellationToken token = default, IProgress<double>? progress = null)
        {
